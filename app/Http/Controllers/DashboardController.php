...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $currentMonthTransactions = Order::where('user_id', $userId)
            ->where('payment_status', 'completed') // Filter untuk status completed
        ...
        $currentMonthItemsSold = $currentMonthTransactions->sum(fn($order) => $order->orderDetails->sum('quantity'));
        $currentMonthTransactionsCount = $currentMonthTransactions->count();

        // Mengambil data dari bulan lalu
        $lastMonthTransactions = Order::where('user_id', $userId)
            ->where('payment_status', 'completed') // Filter untuk status completed
        ...
        $lastMonthItemsSold = $lastMonthTransactions->sum(fn($order) => $order->orderDetails->sum('quantity'));
